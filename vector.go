...
// ProjectPoint returns a 2D project point from a 3D point using Perspective
// Divide.  This currently scales the points from (-1 to 1) to (-800 to
// 800). This would put the points off screen, but the 'perspective divide'
// division scales it back down.
func ProjectPoint(v Vec3) Vec2 {
	projectedPoint := Vec2{
		x: (FOV * v.x) / v.z,
		y: (FOV * v.y) / v.z,
	}
